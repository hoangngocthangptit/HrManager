...
public class PersonalDAO {
    ...
    public static ArrayList<NhanVienChinhThuc> getNhanVienChinhThuc() throws SQLException {
        ...
        String sql = "select * from \n"
                + "openquery(CONNECTION2MYSQL,'select*from employee,`Pay Rates` "
                + "where `Pay Rates_idPay Rates`=`idPay Rates`') as A \n"
...
//cmt
        ...
        while (rs.next()) {
            ...
            nv.setEmployee_ID(Integer.parseInt(rs.getString("idEmployee")));
...
//todo sua
    ...
    public static Employement Get_Employment_By_Id_2_SQLSERVER(int id) throws SQLException {
        Connection con = DBConnect.getConnectionSQLServer();
        PreparedStatement ps = con.prepareStatement("Select * from Employment where Employee_ID = ? ");
        ps.setInt(1, id);
        ...
        while (rs.next()) {
            ...
            e.setEmployment_Status("Employment_Status");
            e.setHire_Date(new SimpleDateFormat("yyyy-MM-dd").format(rs.getDate("Hire_Date")));
            e.setTermination_Date(new SimpleDateFormat("yyyy-MM-dd").format(rs.getDate("Termination_Date")));
        }
        return e;
    }

    public static Personal2 getP(int id) throws SQLException {
        Connection con = DBConnect.getConnectionSQLServer();
        PreparedStatement ps = con.prepareStatement("select*from personal where Employee_ID = '" + id + "' ");
        ResultSet rs = ps.executeQuery();
        Personal2 p = new Personal2();
        while (rs.next()) {
            ...
            p.setShareholderStatus(rs.getInt("Shareholder_Status"));
            p.setEthnicity(rs.getString("Ethnicity"));
            p.setBirthday(new SimpleDateFormat("yyyy-MM-dd").format(rs.getDate("birthday")));
        }
        return p;
    }
    ...
    public static boolean xoaPersonal(int id) throws SQLException {
        Connection con = DBConnect.getConnectionSQLServer();
        PreparedStatement ps = con.prepareStatement("delete from Employment where Employee_ID = " + id + " delete from Job_History where Employee_ID = " + id
                + " delete from Personal where Employee_ID = " + id);
        return ps.executeUpdate() == 1;
    }
...
//tim kiem
//   public static List<NhanVienChinhThuc> Find_Name(String name) throws SQLException {
//       
//        Connection con = DBConnect.getConnectionSQLServer();
//        String sql = "select * from \n"
//                + "openquery(CONNECTION2MYSQL,'select*from employee,`Pay Rates` "
...
//    }
    ...
    public static void main(String[] args) throws SQLException, ParseException {
        ...
        //List<NhanVienChinhThuc>nv=Find_Name("t");
